...
class L10nEsAeatMod322Report(models.AbstractModel):
    ...
    field_61 = fields.Float(
        string="[61] Prorate Regularization",
        default=0,
        compute="_compute_field_61",
        help="Prorate regularization by application of the final percentage.",
    )

    @api.depends("tax_line_ids", "tax_line_ids.amount")
    def _compute_field_61(self):
        for report in self:
            report.field_61 = report.tax_line_ids.filtered(
    ...
    def _prepare_regularization_move_lines_prorate(self):
        ...
        lines = self._process_tax_line_regularization_prorate(
            self.tax_line_ids.filtered(lambda x: x.field_number == 61)
        )
