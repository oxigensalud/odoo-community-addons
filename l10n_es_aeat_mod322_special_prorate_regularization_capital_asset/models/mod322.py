...
class L10nEsAeatMod322Report(models.AbstractModel):
    ...
    field_60 = fields.Float(
        string="[60] Capital Assets Prorate Regularization",
        default=0,
        compute="_compute_field_60",
        help="Capital assets regularization by application of the final percentage.",
    )

    @api.depends("tax_line_ids", "tax_line_ids.amount")
    def _compute_field_60(self):
        for report in self:
            report.field_60 = report.tax_line_ids.filtered(
                lambda x: x.field_number == 60
            ).amount
    ...
    def _calculate_amount(self, assets_to_regularize, tax_final_percentage):
        ...
        for asset in assets_to_regularize:
            ...
            asset_regularization_line = (
                asset.capital_asset_prorate_regularization_ids._get_by_year_322(self)
            )
            if asset_regularization_line:
                ...
                elif asset_regularization_line.mod322_id != self:
                    raise ValidationError(
                        _(
                            "This asset have a prorate regularization"
    ...
    def create_regularization_move(self):
        result = super().create_regularization_move()
    ...
    def _prepare_regularization_prorate_capital_asset(self):
        lines = self._process_tax_line_regularization_prorate_capital_asset(
            self.tax_line_ids.filtered(lambda x: x.field_number == 60)
        )
        asset_counterpart = []
        for line in lines:
            account = (
    ...
    def _check_mod_322_year(self):
        for rec in self:
            prorate_line_years = (
                rec.capital_asset_prorate_regularization_line_ids.filtered(
                    lambda x, _rec=rec: x.year != _rec.year
                )
            )
            if prorate_line_years:
                raise ValidationError(
                    _(
                        "The model 322 is linked in prorate lines in the"
                        " year: %(year)s. Please review the assets: %(assets)s"
                    )
                    % {
                        "year": rec.year,
